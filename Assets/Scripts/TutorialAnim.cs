...
public class TutorialAnim : MonoBehaviour
{
    ...
    private void Update()
    {
        if (!isPlaying || !animCanEnd) return;

        bool hasTouchInput = Input.touchCount > 0;
        bool hasMouseInput = Input.GetMouseButtonDown(0);
        bool hasKeyboardInput = Input.GetKeyDown(KeyCode.Space);

        if (hasTouchInput || hasMouseInput || hasKeyboardInput)
        {
